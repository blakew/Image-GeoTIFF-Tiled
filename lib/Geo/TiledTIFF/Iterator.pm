package Geo::TiledTIFF::Iterator;
use strict;
use warnings;
use Carp;
...
sub new {
    ...
    if ( $opts ) {
        for ( keys %{$opts} ) {
            $self->image($opts->{$_}) if $_ eq 'image';         # Geo::TiledTIFF object
...
sub _verify {
    ...
    confess "Geo::TiledTIFF object required."
        unless defined $image and ref $image and $image->isa('Geo::TiledTIFF');
...
Geo::TiledTIFF::Iterator - A convenience class to iterate through arbitrarily-shaped raster data.
...
=head1 SYNOPSIS
    ...
    my $tiff = Geo::TiledTIFF->new( $tiff_filepath );
...
Just a simple iterator class for buffers generated by C<Geo::TiledTIFF>.
...
The C<Geo::TiledTIFF> object from whence the values came.
