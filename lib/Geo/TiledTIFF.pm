...
SV* _get_x_values(SV* shape, double y) {
    ...
    PUTBACK;
    count = call_method( "Geo::TiledTIFF::Shape::get_x", G_SCALAR );
    SPAGAIN;
    if ( count != 1 )
        croak("Geo::TiledTIFF::Shape::get_x didn't return a sole value.");
    x_values = POPs;
    PUTBACK;

    if ( SvTYPE(SvRV(x_values)) != SVt_PVAV )
        croak("Geo::TiledTIFF::Shape::get_x didn't return a reference to an array.");
    if ( DEBUG >= 1 ) {
...
//SV* extract_2D_array(SV* obj, SV* svx_min, SV* tile_buffer, SV* shape) { 
    ...
    SV* x_values;                       // Returned from Geo::TiledTIFF::Shape::get_x
    ...
    if ( sv_isobject(shape) && sv_isa(shape,"Geo::TiledTIFF::Shape") ) {
