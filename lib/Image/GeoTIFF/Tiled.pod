...
Image::GeoTIFF::Tiled - A Perl interface to libgeotiff for tiled TIFF images, with support for extracting and iterating arbitrary shapes.
...
Image::GeoTIFF::Tiled provides an interface to libtiff and libgeotiff for raster data stored in tiled TIFF format conforming to the GeoTIFF specification. Several additional functions are provided for ease of access to the underlying geodata.
