...
for my $tiff ( qw/ usgs1 usgs2 / ) {
    next if $tiff eq 'usgs1';
    my $image = Geo::TiledTIFF->new( "./samples/$tiff.tif" );
#    for my $shpath ( glob "./samples/${tiff}_*.shp" ) {
#        my $shp = Geo::ShapeFile->new('./samples/' . basename($shpath,'.shp'));
#        my $shape =
...
CIRCLE: {
    ...
    my $shp = Geo::ShapeFile->new('./samples/usgs2_circle');
    my $shape = 
        Geo::TiledTIFF::Shape->load_shape($image,undef,$shp->get_shp_record(1));
    my $iter = $image->get_iterator_shape($shape);
